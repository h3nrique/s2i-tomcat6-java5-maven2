...
LABEL io.k8s.description="Platform for building with Maven2 and running Java 5 applications on Tomcat 6." \
      sun.java.version="6u45" \
      apache.maven.version="2.2.1" \
      apache.tomcat.version="6.0.53" \
      io.openshift.expose-services="8080:http" \
      io.openshift.tags="s2i,builder,tomcat,tomcat6,java,java5,maven,maven2" \
      io.openshift.s2i.destination="/opt/s2i/destination"

ADD java-installer/jdk-1_5_0_22-linux-amd64.* /tmp/

# Install Maven, Tomcat, Java
RUN INSTALL_PKGS="tar unzip bc which lsof" && \
    cd /tmp && \
    sha256sum -c jdk-1_5_0_22-linux-amd64.bin.sha256 && \
    yum repolist && \
    ...
    rm -rfv /tmp/jdk-1_5_0_22-linux-amd64.* \
           "$JAVA_HOME/"*src.zip \
